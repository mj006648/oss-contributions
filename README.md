...
다음 PR로 검토 중인 이슈들. 2026-06-02 GraphQL `closedByPullRequestsReferences` + `timelineItems(CROSS_REFERENCED_EVENT)`로 linked PR 0건 엄격 검증.

**룰**: linked PR 0건, assignee 0명, 코멘트는 사람 코멘트 0건 (stale bot 만은 허용).

| 우선 | 프로젝트 | 이슈 | 작업 종류 | 무엇 | 왜 필요 | 본인 강점 | 위험 | 상태 |
|---|---|---|---|---|---|---|---|---|
| 🟢 1 | Polaris | [#3716](https://github.com/apache/polaris/issues/3716) Some ignored Hibernate Validator constraints (static methods) | 버그픽스 (소규모) | `@Valid`/`@NotNull` 등이 `static` 빌더 메서드에 붙어 무시되는 경고 다수. 어노테이션 위치 조정으로 해결 | Hibernate Validator는 static 메서드 제약 미지원 — 런타임 검증 누락 | Polaris #4451 머지로 reviewer 신뢰 확보. 본인 첫 Polaris **코드** PR로 적절한 범위 | static method 디자인 자체를 손대야 할 수 있음 — 보고된 `#3467` 기법 참조하면 안전 | stale 라벨 (사람 코멘트 0건) / linked PR 0건 / assignee 0명 / 2026-02-09 생성 |
| 🟢 2 | Polaris | [#4302](https://github.com/apache/polaris/issues/4302) Implement validateAccessToLocations for S3 Tables storage integration | 기능 구현 (follow-up) | `AwsS3TablesCredentialsStorageIntegration.validateAccessToLocations()`가 빈 map 반환 no-op. ARN 기반 검증 구현 | PR #4052에서 의도적으로 deferred — follow-up 환영 분위기 | 본인 AWS S3/Iceberg 경험. 이슈 본문에 구현 가이드까지 명시됨 | S3 Tables API 호출 패턴 학습 필요 | stale bot 코멘트만 / linked PR 0건 / assignee 0명 / 2026-04-27 생성 |
| 🟡 3 | Polaris | [#3676](https://github.com/apache/polaris/issues/3676) Extract storage integration code from polaris-core into separate modules per technology | 리팩토링 (대규모) | `polaris-core`가 AWS/GCS/Azure 모든 storage integration 강제 의존 → 다운스트림 빌드 무거움. tech별 모듈 분리 | downstream 빌드 무게 감소. Polaris 모듈 architecture 개선 | Trident에서 polaris-core 의존 경험 — downstream 빌드 무게 직접 체감 | 큰 리팩토링이라 maintainer 디자인 토론 필수. dev ML 통과 필요할 수 있음 | stale bot 코멘트만 / linked PR 0건 / assignee 0명 / 2026-02-04 생성 |
| 🟡 4 | Nessie | [#12429](https://github.com/projectnessie/nessie/issues/12429) [Feature]: add nessie GC to the helm chart | 인프라 (Helm) | 현재 Nessie Helm chart에 Nessie GC 컴포넌트 누락. Helm chart values + templates 추가 | Helm 배포 사용자가 GC 별도 설정 필요 | TwinX 운영 + ArgoCD/Helm 경험 직결 | feature 추가라 maintainer 디자인 의견 가능 | 13일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 5 | Nessie | [#12398](https://github.com/projectnessie/nessie/issues/12398) [Bug]: Metrics Endpoint is Missing Since v0.107.5 | 버그픽스 (회귀) | v0.107.5부터 `/q/metrics` 404. docker run 2줄로 재현 가능 | Quarkus metrics 설정 회귀로 추정 — Nessie 운영자에 즉시 영향 | dimas-b 리뷰 관계 확보 ([[project_trident_baseline_runner]] Nessie 운영 경험) | Quarkus metrics 설정/의존성 위치 파악 필요. 회귀 도입 PR 추적 필요 | 29일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-05-04) |
| 🟡 6 | Iceberg | [#16581](https://github.com/apache/iceberg/issues/16581) SchemaUpdate cannot move a column by its new name after renameColumn | 버그 (question 라벨) | 한 SchemaUpdate 안에서 renameColumn 후 새 이름으로 moveColumn 호출 시 실패 | API 일관성 — 단일 update 내 연쇄 작업 지원 | Iceberg Java core 첫 진입 후보 | question 라벨이라 maintainer 토론 필요. 첫 Iceberg Java PR로 부담 | 6일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🔴 7 | Nessie | [#12363](https://github.com/projectnessie/nessie/issues/12363) Sweep phase deletes live files when multiple content IDs share base location | 버그픽스 (Critical) | GC sweep이 공유 base location에서 live snapshot 파일을 삭제. content-id별 bloom filter 격리 누락 — 데이터 손실 버그 | 데이터 손실 — 이론적으로 가장 임팩트 큰 픽스 | Iceberg/Nessie commit 모델 이해 | **데이터 손실 버그라 책임 크고**, GC 알고리즘 변경은 maintainer가 senior reviewer만 받을 가능성. 첫 Nessie GC PR로는 부담 | 40일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-04-22) |
