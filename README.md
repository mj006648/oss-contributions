...
|---|---|---|---|---|---|---|---|---|
| 🟢 1 | Nessie | [#12398](https://github.com/projectnessie/nessie/issues/12398) [Bug]: Metrics Endpoint is Missing Since v0.107.5 | 버그픽스 (회귀) | v0.107.5부터 `/q/metrics` 404. docker run 2줄로 재현 가능 | v0.107.5 changelog의 "Remove micrometer simpleclient (#12336)"이 유력 회귀 원인 — PR #12336 diff 확인 필요 | dimas-b 리뷰 관계 + Nessie 3 PR 머지로 진입 깊음. Nessie 첫 **코드** PR (이전 docs/CLI 머지와 별개) | PR #12336이 의도된 변경이면 docs/마이그레이션 가이드 PR로 전환 필요 | 29일 fresh / 사람 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-05-04) |
| 🟢 2 | Nessie | [#12429](https://github.com/projectnessie/nessie/issues/12429) [Feature]: add nessie GC to the helm chart | 인프라 (Helm) | 현재 Nessie Helm chart에 Nessie GC 컴포넌트 누락. CronJob + nessie-gc:0.107.5 이미지 활용 | Helm 배포 사용자가 GC 별도 설정 필요 | TwinX 운영 + ArgoCD/Helm 경험 직결 | feature 추가라 maintainer 디자인 토론 가능 (CronJob schedule 기본값, RBAC, values 구조) | 13일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 3 | Iceberg | [#16581](https://github.com/apache/iceberg/issues/16581) SchemaUpdate cannot move a column by its new name after renameColumn | 버그 (question 라벨) | 한 SchemaUpdate 안에서 renameColumn 후 새 이름으로 moveColumn 호출 시 실패. "Cannot move missing column" 에러 | API 일관성 — 단일 update 내 연쇄 작업 지원 여부 명확화 | Iceberg Java core 첫 진입 후보 | question 라벨이라 maintainer 답부터 받아야 — "버그/의도" 확인. 의도면 docs PR로 전환 | 6일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 4 | Polaris | [#3676](https://github.com/apache/polaris/issues/3676) Extract storage integration code from polaris-core into separate modules per technology | 리팩토링 (대규모) | `polaris-core`가 AWS/GCS/Azure 모든 storage integration 강제 의존. tech별 모듈 분리. 기존 `polaris-core/storage/aws,azure,gcp/` 디렉토리 이미 분리되어 모듈화만 남음 | downstream 빌드 무게 감소 | Trident에서 polaris-core 의존 경험 직결 | 큰 리팩토링이라 dev ML 토론 필수. 첫 Polaris 코드 PR로는 부담 | stale bot 코멘트만 / linked PR 0건 / assignee 0명 / 2026-02-04 생성 |
| 🔴 5 | Nessie | [#12363](https://github.com/projectnessie/nessie/issues/12363) Sweep phase deletes live files when multiple content IDs share base location | 버그픽스 (Critical, 보류) | GC sweep이 공유 base location에서 live snapshot 파일을 삭제. 데이터 손실 버그. 재현·원인 분석 완벽 | 데이터 손실 — 이론적 임팩트 최대 | Iceberg/Nessie commit 모델 이해 | GC 알고리즘 (bloom filter 격리 → cross-content-id aggregation) 변경은 senior reviewer 요구. dimas-b/snazy가 직접 처리할 가능성 | 메모만 — 40일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-04-22) |
