...
다음 PR로 검토 중인 fresh 이슈들. 2026-06-02 재정찰:
- 60일 이내 fresh, 코멘트 0건, linked PR 0건, assignee 0명 모두 만족하는 것만 유지.

| 우선 | 프로젝트 | 이슈 | 작업 종류 | 무엇 | 왜 필요 | 본인 강점 | 위험 | 상태 |
|---|---|---|---|---|---|---|---|---|
| 🟢 1 | Polaris | [#4594](https://github.com/apache/polaris/issues/4594) Do not instantiate MetricsPersistence for InMemoryBufferEventListener | 리팩토링 (소규모) | `InMemoryBufferEventListener` 생성 시 불필요한 `MetricsPersistence` 인스턴스화 제거 | `InMemoryBuffer`는 DB persistence 불필요 — PR #4397 리뷰에서 maintainer가 직접 이슈로 분리 | Polaris #4451 머지로 reviewer(dimas-b/flyrain) 신뢰 확보. 범위 최소 — 단일 클래스 수정 | 없음. 이슈 본문에 이미 타깃 PR discussion 링크 명시 | 1일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-01) |
| 🟢 2 | Polaris | [#4553](https://github.com/apache/polaris/issues/4553) Granting identical catalog roles is not idempotent for JDBC-backed persistence | 버그픽스 | PUT grant 중복 요청 시 HTTP 500 반환. `JdbcBasePersistenceImpl.writeToGrantRecords`에서 duplicate key 예외가 그대로 클라이언트에 노출 | HTTP PUT 멱등성 위반. in-memory/NoSQL 백엔드는 이미 멱등성 있음 — JDBC만 누락 | Polaris 코드베이스 친숙. JDBC upsert 패턴은 재현·범위 명확 | JDBC 트랜잭션 처리 세부 확인 필요. retry 로직과의 상호작용 점검 | 6일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-05-27) |
| 🟢 3 | Nessie | [#12429](https://github.com/projectnessie/nessie/issues/12429) [Feature]: add nessie GC to the helm chart | 인프라 (Helm) | 현재 Nessie Helm chart에 Nessie GC 컴포넌트 누락. Helm chart values + templates 추가 | Helm 배포 사용자가 GC 별도 설정 필요 — 운영 불편 | TwinX 운영 + ArgoCD/Helm 경험 직결 | feature 추가라 maintainer 디자인 의견 가능. dev ML 토론 필요 가능성 | 13일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 4 | Iceberg | [#16542](https://github.com/apache/iceberg/issues/16542) Flaky TestSnapshotSummary#testFileSizeSummaryWithDVs | 테스트 안정화 | `FileGenerationUtil.generateContentLength()`가 `nextInt(10_000)` → 0 반환 가능. DV `contentSizeInBytes==0`일 때 `added-files-size` 필드 스킵되어 assertion 실패 | flaky test는 CI 신뢰도 저하. 수정은 `nextInt(1, 10_000)` 한 줄 | 원인 완전히 분석됨. Iceberg Java 첫 PR로 부담 최소 | 없음 | 10일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 5 | Iceberg | [#16601](https://github.com/apache/iceberg/issues/16601) Kafka Connect: KafkaMetadataTransform leaks configuration via static field | 버그픽스 | `KafkaMetadataTransform.recordAppender`가 `static` 선언 — 인스턴스 간 설정 공유로 잘못된 SMT 동작 | 멀티 커넥터 환경에서 설정 오염. Java 버그이며 원인·수정 명확 | Kafka Connect 도메인 인접. static → instance field 변경 + 테스트 추가 | 회귀 방지 테스트 작성 필요 | 4일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-05-29) |
| 🟡 6 | Polaris | [#4492](https://github.com/apache/polaris/issues/4492) Add support for unique-table-location property | 기능 추가 | Iceberg 1.11 신규 카탈로그 프로퍼티 `unique-table-location` Polaris 미지원. `CatalogTests.createTableInUniqueLocation()` 실패 | Iceberg 1.11 spec 준수 — 테스트 케이스까지 이슈에 명시됨 | Polaris 카탈로그 레이어 친숙. Iceberg 1.11 upstream PR #12892 참조 가능 | feature 추가라 scope 논의 필요 가능성 | 14일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-05-19) |
| 🟡 7 | iceberg-python | [#3388](https://github.com/apache/iceberg-python/issues/3388) fix(streaming-write): rolling ParquetWriter + OutputStream.tell() | 버그픽스 | 스트리밍 쓰기 시 파일 사이즈 spec 위반 + 메모리 무경계. rolling ParquetWriter + `OutputStream.tell()` 도입 | 스트리밍 쓰기 안정성 + Iceberg spec 준수 | Trident ingest 영역 인접 | 스트리밍 쓰기 경로 변경은 회귀 위험 — 테스트 커버리지 확보 필요 | 13일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 8 | Iceberg | [#16581](https://github.com/apache/iceberg/issues/16581) SchemaUpdate cannot move a column by its new name after renameColumn in the same update | 버그 (question 라벨) | 한 SchemaUpdate 안에서 renameColumn 후 새 이름으로 moveColumn 호출 시 실패 | API 일관성 — 단일 update 내 연쇄 작업 지원 | Iceberg Java core 첫 진입 후보 | question 라벨이라 maintainer 토론 필요. 첫 Iceberg Java PR로 부담 | 6일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
| 🟡 9 | Iceberg | [#16530](https://github.com/apache/iceberg/issues/16530) Flaky tests in iceberg-azure integrationTest | 테스트 안정화 | `TestADLSFileIO` / `TestADLSInputStream`이 testcontainer 환경에서 간헐 실패. CI 로그상 `ContainerFetchException` | flaky test는 CI 신뢰도/머지 속도 저하 | Docker/testcontainer/K8s 운영 경험 | flaky 디버깅은 재현성이 까다로워 PR 임팩트 입증 핵심 | 11일 fresh / 코멘트 0건 / linked PR 0건 / assignee 0명 (2026-06-02) |
...
| 프로젝트 | 이슈 | 회피 이유 |
|---------|------|----------|
| iceberg-python | [#3410](https://github.com/apache/iceberg-python/issues/3410) refactor sigv4 out of rest catalog | 코멘트 1건 발생 (2026-06-02 확인) — 조건 위반. 재진입 시 코멘트 내용 확인 후 판단 |
